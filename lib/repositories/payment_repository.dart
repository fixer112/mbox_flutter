...
class PaymentRepository {
  ...
  Future<OrderCreateResponse> getOrderCreateResponse(
    ...
    var post_body = jsonEncode({"owner_id": "${owner_id}", "user_id": "${user_id.value}","payment_type": "${payment_method}"});
  ...
  Future<PaypalUrlResponse> getPaypalUrlResponse(
      @required String payment_type ,@required int order_id,@required double amount) async {

    final response = await http.get("${AppConfig.BASE_URL}/paypal/payment/url?payment_type=${payment_type}&order_id=${order_id}&amount=${amount}&user_id=${user_id.value}",
        );
  ...
  Future<OrderCreateResponse> getOrderCreateResponseFromWallet(
      @required int owner_id ,@required payment_method,@required double amount) async {

    var post_body = jsonEncode({"owner_id": "${owner_id}", "user_id": "${user_id.value}","payment_type": "${payment_method}","amount": "${amount}"});

    final response = await http.post("${AppConfig.BASE_URL}/payments/pay/wallet",
        headers: {"Content-Type": "application/json", "Authorization": "Bearer ${access_token.value}"},body: post_body );
